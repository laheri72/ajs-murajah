...
export function TargetsPage() {
  ...
  return (
    <div className="grid gap-5">
      <AdminPageHeader title="Targets" description="Set weekly, monthly, and yearly Rub' expectations for every room." />
      ...
      <section className="grid gap-4 xl:grid-cols-[380px_1fr]">
          ...
          <form className="mt-4 grid gap-4" onSubmit={onSubmit}>
            ...
            <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
              <div>
                ...
                <Input type="number" min={1} max={120} value={form.yearly_rub_target} onChange={(event) => setForm({ ...form, yearly_rub_target: Number(event.target.value) })} />
              </div>
              <div>
                <Label>Weekly Rub'</Label>
                <Input type="number" min={0} max={120} value={form.weekly_rub_target} onChange={(event) => setForm({ ...form, weekly_rub_target: Number(event.target.value) })} />
              </div>
              <div>
                <Label>Monthly Rub'</Label>
                <Input type="number" min={0} max={120} value={form.monthly_rub_target} onChange={(event) => setForm({ ...form, monthly_rub_target: Number(event.target.value) })} />
              </div>
            </div>
        ...
        <Card className="p-0">
          <div className="border-b border-border p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Each room should have one active target profile.</p>
          </div>
